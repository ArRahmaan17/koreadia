...
class AgendaController extends Controller
{
    ...
    {
        return view('master.mail-agenda');
    }

    public function dataTable(Request $request)
    {
        $totalData = MailAgenda::orderBy('id', 'asc')
            ->count();
        ...
        if (empty($request['search']['value'])) {
            ...
            $totalFiltered = MailAgenda::select('*')
                ->where('name', 'like', '%' . $request['search']['value'] . '%')
                ->orWhere('description', 'like', '%' . $request['search']['value'] . '%');
    ...
    public function all()
    {
        $response = ['message' => 'showing all resources successfully', 'data' => MailAgenda::all()];
        $code = 200;
        if (MailAgenda::count() == 0) {
            $response = ['message' => 'failed showing all resources', 'data' => MailAgenda::all()];
            $code = 422;
        }
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            MailAgenda::create($request->except('_token'));
    ...
    public function update(Request $request, string $id)
    {
        ...
        DB::beginTransaction();
        try {
            MailAgenda::find($id)->update($request->except('_token'));
