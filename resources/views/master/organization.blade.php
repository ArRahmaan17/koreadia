...
@section('content')
    ...
    <div class="card">
        <div class="card-header align-items-center d-flex">
            <h4 class="card-title mb-0 flex-grow-1">@lang('translation.organization')</h4>
            <div class="flex-shrink-0">
                <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#modal-organization">@lang('translation.add')</button>
            </div>
        </div><!-- end card header -->
        <div class="card-body">
            <div class="table-responsive p-2">
                <table class="table table-bordered" id="table-organization">
                    <thead>
                        <tr>
    ...
    <div id="modal-organization" class="modal fade" tabindex="-1" aria-labelledby="modal-organization-label" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-organization-label">@lang('translation.add') @lang('translation.organization')</h5>
                ...
                <div class="modal-body">
                    <form action="#" id="form-organization">
                        ...
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" id="name" name="name" placeholder="Enter your name organization mail">
                            <label for="name">@lang('translation.name')</label>
                        </div>
                        <div class="form-floating mb-3">
                            <textarea class="form-control" id="description" name="description" rows="3" placeholder="Enter your description organization mail"></textarea>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-soft-success" id="save-organization">@lang('translation.save') @lang('translation.changes')</button>
                    <button type="button" class="btn btn-soft-warning d-none" id="update-organization">@lang('translation.update') @lang('translation.changes')</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function actionData() {
            $('.edit').click(function() {
                ...
                $('#save-organization').addClass('d-none');
                $('#update-organization').removeClass('d-none');

                $.ajax({
                    type: "GET",
                    url: "{{ route('master.organization.show') }}/" + idOrganization,
                    dataType: "json",
                    success: function(response) {
                        $('#modal-organization').find("form")
                ...
                iziToast.question({
                    ...
                    buttons: [
                        ['<button><b>OK</b></button>', function(instance, toast) {
                            ...
                            $.ajax({
                                type: "DELETE",
                                url: "{{ route('master.organization.destroy') }}/" +
                                    idOrganization,
                                data: {
                                    _token: `{{ csrf_token() }}`,
                                },
        ...
        $(function() {
            ...
            $('#save-organization').click(function() {
                let data = serializeObject($('#form-organization'));
                $.ajax({
                    type: "POST",
                    url: `{{ route('master.organization.store') }}`,
                    data: data,
                    dataType: "json",
                    success: function(response) {
                    ...
                    error: function(error) {
                        $('#modal-organization .is-invalid').removeClass('is-invalid')
                        $.each(error.responseJSON.errors, function(indexInArray,
                            valueOfElement) {
                            $('#modal-organization').find('[name=' + indexInArray +
                                ']').addClass('is-invalid')
                        });
                        iziToast.error({
                            id: 'alert-organization-form',
            ...
            $('#update-organization').click(function() {
                let data = serializeObject($('#form-organization'));
                $.ajax({
                    type: "PUT",
                    url: `{{ route('master.organization.update') }}/${data.id}`,
                    data: data,
                    dataType: "json",
                    success: function(response) {
                        $('#modal-organization').modal('hide')
            ...
            $('#modal-organization').on('hidden.bs.modal', function() {
                ...
                $(this).find('.modal-title').html(`Add @lang('translation.organization')`);
                $('#save-organization').removeClass('d-none');
                $('#update-organization').addClass('d-none');
                $('#modal-organization .is-invalid').removeClass('is-invalid')
                $('#table-organization tbody').find('tr').removeClass('selected');
            });
        });
